    ...
    try:
        ...
        if action_type == "create_file":
            ...
            for change in action_element.findall("./change"):
                original = (
                    change.find("original").text
                    if change.find("original") is not None
                    else ""
                )
        ...
        confirm = Prompt.ask("\nExecute this action?", choices=["Y", "n"], default="Y")
        ...
        if action_type == "create_file":
            ...
            for change in action_element.findall("./change"):
                ...
                if original in file_content:
                    ...
                    console.print(
                        f"[bold yellow]Warning: Could not find text to replace in {path}[/bold yellow]"
                    )
            ...
            if changes_applied:
                ...
                console.print(
                    f"[bold yellow]No changes applied to {path}[/bold yellow]"
                )
                return False
            ...
            console.print(
                f"[bold yellow]Unknown action type: {action_type}[/bold yellow]"
            )
            return False
...
def execute_file_edit(edit_element, console: Console) -> bool:
    ...
    try:
        ...
        if search_elem is None or replace_elem is None:
            console.print(
                "[bold red]Error: Missing search or replace elements[/bold red]"
            )
            return False
        ...
        # Ask for confirmation
        confirm = Prompt.ask("\nApply this edit?", choices=["Y", "n"], default="Y")
        ...
        if search_text == "":
            ...
            if search_text not in content:
                console.print(
                    f"[bold yellow]Warning: Search text not found in {path}[/bold yellow]"
                )
                return False
...
def execute_shell_command(
    command: str, console: Console, auto_run: bool = False
) -> Tuple[bool, str]:
    ...
    if not auto_run:
        confirm = Prompt.ask(
            "\nExecute this shell command?", choices=["Y", "n"], default="Y"
        )
            ...
            if completed_task_id in dependencies:
                ...
                if all_deps_completed:
                    ...
                    if current_status == "pending":
                        ...
                        console.print(
                            f"[bold blue]Task {task.get('id')} is now ready to be executed[/bold blue]"
                        )
