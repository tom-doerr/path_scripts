...
from src.interface.actions import (
    execute_action,
    execute_file_edit,
...
def process_chat_response(
    ...
    if getattr(agent, "config", {}).get("verbose", False):
        ...
        try:
            ...
            console.print(
                Syntax(formatted_xml, "xml", theme="monokai", line_numbers=True)
            )
    ...
    chat_history.append(
        {"role": "assistant", "content": history_content, "timestamp": timestamp}
    )
    ...
    if actions_xml:
        try:
            ...
            if actions:
                console.print(
                    "[bold blue]The model suggests the following actions:[/bold blue]"
                )
    ...
    if file_edits_xml:
        try:
            ...
            if edits:
                console.print(
                    "[bold blue]The model suggests the following file edits:[/bold blue]"
                )
    ...
    if shell_commands_xml:
        try:
            ...
            if commands:
                console.print(
                    "[bold blue]The model suggests the following shell commands:[/bold blue]"
                )

                for cmd_elem in commands:
                    command = cmd_elem.text.strip() if cmd_elem.text else ""
                    auto_run = (
                        cmd_elem.get("safe_to_autorun", "false").lower() == "true"
                    )

                    if auto_run:
                        console.print(
                            f"[bold green]Auto-running safe command:[/bold green] {command}"
                        )
                        success, context_xml = execute_shell_command(
                            command, console, auto_run=True
                        )
                    else:
                        success, context_xml = execute_shell_command(
                            command, console, auto_run=False
                        )
    ...
    if execution_status_xml:
        try:
            ...
            console.print(
                f"[bold red]Error parsing execution status XML: {e}[/bold red]"
            )
