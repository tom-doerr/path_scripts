...
class AgentInterface:
        ...
        try:
            if os.path.exists(self.history_file):
                with open(self.history_file, "r") as f:
                    ...
                    self.console.print(
                        f"[dim]Loaded {len(self.chat_history)} previous messages[/dim]"
                    )
    ...
    def chat_with_model(self, message: str):
        ...
        process_user_input(
            self.agent, message, self.chat_history, self.history_file, self.console
        )
                ...
                if self.multiline_input_mode:
                    ...
                    if user_input.lower() in ["/end", "/done", "/finish"]:
                        ...
                        if self.multiline_input_buffer:
                            ...
                            self.console.print(
                                f"[dim]Processing {len(self.multiline_input_buffer)} lines of input...[/dim]"
                            )
                        ...
                        self.console.print(
                            f"[dim]Line {len(self.multiline_input_buffer)} added[/dim]"
                        )
                else:
                    ...
                    if user_input.startswith("/"):
                        ...
                        if "\n" in user_input:
                            ...
                            self.console.print(
                                f"[dim]Detected multiline paste with {len(lines)} lines[/dim]"
                            )
