...
def display_help(console: Console):
    ...
    console.print(
        "[bold green]/plan [spec_file][/bold green]",
        "- Generate a plan from specification (default: spec.md)",
    )
    console.print("[bold green]/display[/bold green]", "- Display the current plan")
    console.print(
        "[bold green]/update <task_id> <status> [--notes=text] [--progress=0-100][/bold green]",
        "- Update task status",
    )
    console.print(
        "[bold green]/execute <task_id>[/bold green]",
        "- Execute a specific task (with confirmation)",
    )
    console.print(
        "[bold green]/execute-ready[/bound green]",
        "- Execute all ready tasks in dependency order",
    )
    console.print(
        "[bold green]/modify-plan[/bold green]",
        "- Let the agent suggest modifications to the plan",
    )
    console.print(
        "[bold green]/model [model_name][/bold green]",
        "- Change the model (aliases: flash, r1, claude)",
    )
    console.print(
        "[bold green]/models[/bold green]", "- List available models and aliases"
    )
    console.print(
        "[bold green]/history [count][/bold green]",
        "- Display chat history (default: last 10 messages)",
    )
    console.print(
        "[bold green]/clear-history[/bold green] or [bold green]/reset[/bold green]",
        "- Clear the chat history",
    )
    console.print("[bold green]/memory[/bold green]", "- Display the agent's memory")
    console.print(
        "[bold green]/clear-memory[/bold green]", "- Clear the agent's memory"
    )
    console.print(
        "[bold green]/paste[/bold green]",
        "- Enter multiline paste mode (end with /end)",
    )
    ...
    console.print("")
    console.print(
        "[italic]Any text not starting with / will be sent directly to the model as a chat message[/italic]"
    )
    console.print(
        "[italic]For multiline input, use /paste to start and /end to finish[/italic]"
    )
    console.print("")
    ...
    console.print("")
    console.print(
        "[bold cyan]<user_message>Your message here</user_message>[/bold cyan]"
    )
    console.print(
        '[bold cyan]<file_request path="path/to/file.py">Request to see a file</file_request>[/bold cyan]'
    )
        ...
        def add_tasks(parent_element, parent_tree):
            for task in parent_element.findall("./task"):
                ...
                if progress and progress.isdigit():
                    ...
                    progress_bar = "█" * (progress_int // 10) + "░" * (
                        10 - (progress_int // 10)
                    )
                    task_text += (
                        f" [{status_color}]{progress_bar} {progress}%[/{status_color}]"
                    )
